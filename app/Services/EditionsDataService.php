...
class EditionsDataService
{
    ...
    {
        // Get the authenticated user's favorited liveset IDs (if logged in)
        $favoritedLivesetIds = [];
        if (Auth::check()) {
        ...
        return Edition::all()->map(function ($edition) use ($timetablerService, $favoritedLivesetIds) {
            return [
                ...$edition->toArray(),
                'date' => $edition->date?->format('j M Y'),
                'livesets' => $edition->livesets->sortBy('lineup_order')->map(function (Liveset $liveset) use ($timetablerService, $edition, $favoritedLivesetIds) {
                    return [
                        ...$liveset->toArray(),
                        ...
                        'is_favorited' => in_array($liveset->id, $favoritedLivesetIds),
                    ];
                })->values(),
            ];
        })->toArray();
