...
class UtilController extends Controller
{
    public function parseCue(Request $request)
    {
        ...
        foreach ($content as $line) {
            ...
            } elseif (preg_match('/^PERFORMER "(.+)"$/', $line, $matches)) {
                $top_lines['performer'] = $matches[1];
            } elseif (preg_match('/\tTRACK \d+/', $line)) {
                // New track line. Bump the song index
                $current_song_index++;
            } elseif (preg_match('/\t\tTITLE "(.+)"/i', $line, $matches)) {
            ...
            } elseif (preg_match('/\t\tPERFORMER "(.+)"/i', $line, $matches)) {
                $songs[$current_song_index]['artist'] = $matches[1];
            } elseif (preg_match('/\t\tINDEX\s\d+\s(\d{2}:\d{2}:\d{2})$/', $line, $matches)) {
